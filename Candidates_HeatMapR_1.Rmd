...
```{r, "get text files", echo=FALSE, warning=FALSE, message=FALSE}
    ...
    ## create dummy file 
    d_all <- d.file[1] %>% load_debate_text
...
```{r, "get candidate text and create text corpora", echo=FALSE, warning=FALSE}
    ...
    ## Get Candidate Text
    pence_text <- "TRUMP" %>% candidate_text(d_all, word.filter)
    cruz_text <- "CRUZ" %>% candidate_text(d_all, word.filter)
    moderator_text <- "QUIJANO" %>% candidate_text(r_all, word.filter)
